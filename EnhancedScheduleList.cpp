...
void EnhancedScheduleList::addFreight(shared_ptr<FreightExtended> freight) {
    freights.push_back(freight);
}

void EnhancedScheduleList::addCargoGroup(const CargoGroup& group) {
    cargoGroups.push_back(group);
}

void EnhancedScheduleList::scheduleByArrivalTime() {
...
void EnhancedScheduleList::scheduleByFreightCapacity() {
...
bool EnhancedScheduleList::canAssignToFreight(const FreightExtended& freight, const Cargo& cargo) const {
    return freight.canAcceptMore() &&
        freight.getDest() == cargo.getDest() &&
        abs(freight.getTime() - cargo.getTime()) <= 15; // Within 15-minute window
}

void EnhancedScheduleList::splitAndAssignGroup(const CargoGroup& group) {
    for (const auto& cargo : group.getCargos()) {
        bool assigned = false;
        for (auto& freight : freights) {
            if (canAssignToFreight(*freight, cargo)) {
...
void EnhancedScheduleList::displayByArrivalTime() const {
...
void EnhancedScheduleList::displayByFreightCapacity() const {
    vector<shared_ptr<FreightExtended>> sortedFreights = freights;
    sort(sortedFreights.begin(), sortedFreights.end(),
        [](const auto& a, const auto& b) {
            return a->getMaxCapacity() > b->getMaxCapacity();
...
void EnhancedScheduleList::displayUnderutilizedFreights() const {
...
void EnhancedScheduleList::displayUnassignedCargos() const {
    cout << "\nUnassigned Cargos:\n";
    cout << "=================\n";
...
void EnhancedScheduleList::saveEnhancedSchedule(const string& filename) const {
...
void EnhancedScheduleList::matchFreightAndCargo(FStorage& fStorage, CStorage& cStorage) {
    matches.clear();
    for (const auto& freight : freights) {
...
void EnhancedScheduleList::printAll() const {
    cout << "Enhanced Schedule List:\n";
    cout << "=======================\n";
    for (const auto& freight : freights) {
